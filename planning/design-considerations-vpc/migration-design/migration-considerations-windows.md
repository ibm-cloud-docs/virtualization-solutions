...
4. First boot in VPC:
   ...
   - Might require rejoining the domain
...
- Might trigger Windows reactivation
...
1. Run virt-v2v:
   ...
   Parameters:
   - `-i disk`: The input is a disk image file
...
- Preserves machine identity (no reactivation, no redomain-join)
...
- Requires clean shutdown (do not process crashed/forced-off virtual machines)
...
- RHEL libguestfs include virtio-win drivers at `/usr/share/virtio-win`
- Ubuntu libguestfs do not include virtio-win drivers
...
2. Identify WinRE location through `reagentc /info`
...
| Production stand-alone servers | virt-v2v (preserves identity) |
