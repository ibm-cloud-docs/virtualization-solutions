...
This tutorial focuses on using RackWare RMM (RackWare Management Module) to migrate IBM Cloud VCF-Automated Virtual Machines (VMs) to IBM Cloud VPC Virtual Server Instances (VSIs). There is an associated Technical Guide that describes the technologies used.

While RMM has an auto-provision feature to automatically provision an IBM Cloud VPC VSI, appropriately sized, this feature was **not used** in this tutorial. Therefore, in this guide we manually provision the target VSIs and associated data disk.

RackWare's RMM Server Migration solution provides an easy, automated, and simplified process to migrate existing IBM Cloud VMware VCF-Automated virtual machines (VM) from their current location to IBM Cloud VPC virtual server instances (VSI).

In this tutorial we migrate an Ubuntu VM and an a Microsoft Windows 2019 VM. This tutorial assumes that you have:

1. Read the associated Technical Guide.
2. An existing IBM Cloud VMware-Automated instance and hosting VMs on NSX overlay segments. These overlay segments do not have native access to the IBM Cloud Classic private network.
3. Created one or more resource groups.
4. Created a VPC with prefixes that cover your required networks.
5. Created a subnet for the RMM server.
6. Created an SSH key and uploaded the public key to VPC SSH keys.
7. Provisioned an RMM server from the IBM catalog.

The RackWare Management Module (RMM) migration solution provides a seamless virtual-to-virtual re-platforming for these VMware VMs to IBM Cloud VSI migration. Its intuitive GUI allows you to move the OS, application, and data from VMware ESXi to IBM Cloud VPC VSI.

1. Create Source VMs.
2. RMM Install:
      1. Create a VPC.
      2. Create subnet for the RMM server.
      3. Create a SSH key.
      4. Deploy the RMM server from the IBM Cloud catalog.
3. Obtain Licensing from RackWare.
4. Create a Transit Gateway with connections to Classic and VPC.
5. Order a portable private subnet.
6. Order a static private subnet.
7. Deploy a VM for use as a bridge server.
8. Configure bridge server.
9. Generate the RMM’s ssh keys
10. Gather information about the source VMs.
11. Order the target VSIs.
12. Prepare the source VM.
13. Use the RMM GUI to configure the migrations and perform the initial migration.
14. Verification.
15. Perform delta syncs.
16. Cut-over.

## Step 1: Create Source VMs
{: #virt-sol-vpc-migration-design-rmm-tutorial-step1}

### Ubuntu Source VM
{: #virt-sol-vpc-migration-design-rmm-tutorial-step1-ubuntu}

Deploy a VM on the IBM Cloud VCF-Automated instance with the following specification:
...
A VM was deployed on the IBM Cloud VCF-Automated instance with the following specification:
...
No second disk was provisioned for this VM.

## Step 2: RMM Install
{: #virt-sol-vpc-migration-design-rmm-tutorial-step2}
...
{: #virt-sol-vpc-migration-design-rmm-tutorial-step3}

1. Obtain licenses from RackWare by emailing the generated preinstall file to RackWare licensing.
2. To generate a preinstall file in `/etc/rackware`, run the following command, **note your public key name an IP address will be different**:
...
3. Copy the file from `/etc/rackware` and send it to `licensing@rackwareinc.com` as an attachment.

```bash
scp -i ~/.ssh/sno3 root@161.156.171.81:'/etc/rackware/rwlicense_preinstall_*' /Work/2025/RMM/
```

4. After receiving a valid license, download the license file to `/etc/rackware` and restart the services to apply the license by running the following command, note your filename will be different:
...
5. Verify the license by running the following command and return the output to `licensing@rackwareinc.com`:

```bash
rw rmm show
```

## Step 4: Create a Transit Gateway with connections to Classic and VPC
{: #virt-sol-vpc-migration-design-rmm-tutorial-step4}
...
Using the IBM Cloud documentation create a local transit gateway and connect your VPC and your IBM Cloud Classic network.

## Step 5: Order a portable private subnet
{: #virt-sol-vpc-migration-design-rmm-tutorial-step5}
...
It is good practice to order a new portable private subnet and assign IP addresses to VMs as needed. This subnet will be used to host the outside interfaces of the one or more RMM bridge servers, therefore, only a small subnet is required.

Using the IBM Cloud documentation order a new portable private subnet in the IBM Cloud Private VLAN named `Private management VLAN`.

Assign an IP address for the RMM Bridge Server deployed later in this tutorial.

## Step 6: Order a static private subnet
{: #virt-sol-vpc-migration-design-rmm-tutorial-step6}
...
Using the IBM Cloud documentation order a new static private subnet in the IBM Cloud Private VLAN named `Private management VLAN` and targeting the IP address in the portable private subnet that you will use for the RMM Bridge Server.

## Step 7: Deploy a VM for use as a bridge server
{: #virt-sol-vpc-migration-design-rmm-tutorial-step7}
...
To access your Ubuntu VM via SSH you may need to configure SSH:
...
Using **Netplan** (default on Ubuntu 18.04+), edit your network config:
...
## Step 8: Configure bridge server
{: #virt-sol-vpc-migration-design-rmm-tutorial-step8}

This example uses an Ubuntu VM hosted on the IBM Cloud VCF-Automated instance. The VM is deployed with two interfaces.
...
**Configure Static NAT (1-to-1 mapping)**
...
## Step 9: Generate the RMM’s ssh keys
{: #virt-sol-vpc-migration-design-rmm-tutorial-step9}
...
2. From the RMM’s console window generate an SSH key pair.
...
Record the public key `cat ~/.ssh/id_rsa.pub`:
...
## Step 10: Gather Information about the Source VMs
{: #virt-sol-vpc-migration-design-rmm-tutorial-step10}
...
2. Follow the IBM Cloud instructions to create the required security group in the VPC for the target VSIs. Ensure that SSH is allowed from the RMM to the target VSIs. For the Windows VSI we will also need to include RDP so that we can connect to the VSI to configure it ready for use by RMM
3. Follow the IBM Cloud instructions to create the required SSH keys in the VPC for the target VSIs. As we are provisioning the Ubuntu Target VSI manually, we will need to upload the RMM public key we created earlier and include this when we provision the VSI. This enables RMM to use password-less SSH. For the Windows VSI, this key is used to encrypt the passowrd
4. With the information gathered in the previous step and using the IBM Cloud documentation order the IBM Cloud VPC VSI servers and associated data volumes.

### Ubuntu Target VSI
{: #virt-sol-vpc-migration-design-rmm-tutorial-step11-ubuntu}

As we are provisioning the Ubuntu Target VSI manually, ensure you have uploaded the RMM public key we created earlier and include this when we provision the VSI. This enables RMM to use password-less SSH.

### Windows Target VSI
{: #virt-sol-vpc-migration-design-rmm-tutorial-step11-windows}

The RMM replicates and syncs Windows servers without requiring a windows password. This is referred to as SSH-Only. To properly configure SSH on Windows the RMM provides a small MSI that needs to be installed on the source VM and target VSI.
...
The RMM SSHD msi installer package can be run either from a web browser or directly on the Windows VSI. After adding the route to the RMM server, we can use `https://<your-RMM-IP-or-FQDN-Address>/windows/RWSSHDService_x64.msi` e.g. `https://10.68.70.11/windows/RWSSHDService_x64.msi`.

For the Target VSI, you will need to get the password after it has been provisioned to install the SSHD. Use the CLI command after logging in, `ibmcloud is in-init <target_VSI_name> --private-key @<private_key_location>` e.g. `ibmcloud is in-init rmm-source2-tgt --private-key @~/.ssh/sno3`
...
To install Rackware SSHD Service in the target VSI:

1. Once the “Welcome to the Rackware SSHD Service Setup Wizard” page is displayed, press Next to begin the installation.
2. Read and accept the “License Agreement”, press Next, and the Installation Folder window will be shown.
3. Then press the ‘Next’ button. The SSHD Configuration window will be shown.
4. On the SSHD Configuration window:
5. As RMM will be accessing the Windows host as the SYSTEM user, then the username field should show SYSTEM.
6. Enter the RMM’s public SSH ke, which is the contents of the file /root/.ssh/id_rsa.pub on the RMM server.
7. Then press Next and the Confirm Installation screen will be shown.
8. Press Next to begin the installation.
9. After the installation completes, you will see the Installation Complete window.
10. Press the Close button.

Alternatively, if you want to use the command line, in a CMD window in the target VSI:
...
## Step 12: Prepare the Source VMs
{: #virt-sol-vpc-migration-design-rmm-tutorial-step12}

### Ubuntu Source VM
{: #virt-sol-vpc-migration-design-rmm-tutorial-step12-ubuntu}

Before a Linux VM can be migrated, it must be configured such that the RMM server can communicate with it and that it can create snapshots from which the RMM will copy the data from the live server.
...
10. Test connectivity between your RMM and the source VM e.g. `ssh rackware@10.194.177.82`. You should connect without being prompted for a password.

### Windows Source VM
{: #virt-sol-vpc-migration-design-rmm-tutorial-step12-windows}

The RMM replicates and syncs Windows servers without requiring a windows password. This is referred to as SSH-Only. To properly configure SSH on Windows the RMM provides a small MSI that needs to be installed on the source VM and target VSI.
The RMM should ssh to the Windows servers as the `SYSTEM` user, this is the user most commonly used by RackWare customers, and should be considered the default choice

The RMM SSHD msi installer package can be run either from a web browser or directly on the Windows VM. After adding the route to the RMM server, we can use `https://<your-RMM-IP-or-FQDN-Address>/windows/RWSSHDService_x64.msi` e.g. `https://10.68.70.11/windows/RWSSHDService_x64.msi`.

On the source VM you will need to add a static route, use the following syntax `route add destination_network MASK subnet_mask gateway_ip metric_cost` where: 
...
To install Rackware SSHD Service in the source VM:
...
Alternatively, if you want to use the command line, in a CMD window in the source VM:
...
## Step 13: Use the RMM GUI to configure the migrations and perform the initial migration
{: #virt-sol-vpc-migration-design-rmm-tutorial-step13}
...
2. **Verification Phase** - Test and verify applications in the target environment
3. **Delta Sync(s)** - Multiple delta syncs can be performed to keep target up-to-date with only changed files
4. **Final Sync/Cutover** - The actual cutover with minimal downtime. It is recommended that any applications are quiesced, while performing the final delta sync

Once the source VMs has been prepared and the RMM can ssh to them, and the information for the target VSIs has been obtained, the VM's workload can be migrated from the source VMs to the target VSIs using the RMM GUI.
...
2. Use `admin` as the Username. `rackware` is the default Password. If you have not already changed the password for the `admin` user, do so by using the standard Linux ‘passwd’ command. Press the Login button. The RMM home page will then be shown.
3. This step is only required, if you are using the auto-provisioning feature. Navigate to Configuration, Environments and click Add Environment.
...
### Creating a wave and replication for the Ubuntu VM
{: #virt-sol-vpc-migration-design-rmm-tutorial-step13-ubuntu}
...
### Creating a wave and replication for the Windows VM
{: #virt-sol-vpc-migration-design-rmm-tutorial-step13-windows}
...
{: #virt-sol-vpc-migration-design-rmm-tutorial-step14}

The replications should complete. Once completed, you can connect to the target VSIs and carryout any testing as needed. The source VMs are still powered on and functioning. The target VSIs are isolated and can be tested without disrupting any production services.

## Step 15: Perform delta syncs
{: #virt-sol-vpc-migration-design-rmm-tutorial-step15}

Delta syncs from the source environment to the IBM Cloud VPC VSIs capture changes to the source VMs since the initial replication was performed.

## Step 16: Cut-over
{: #virt-sol-vpc-migration-design-rmm-tutorial-step16}

In this step we quiesce the applications and perform a cut-over/final delta sync from the source VMs to the IBM Cloud VPC VSIs.

1. Quiesce the applications on the source VMs. This step minimizes I/O during the final sync and ensures data consistency:
      - Stop applications/databases on the source VMs
      - Log out other users from the system
      - For systems with databases, ensure applications flush outstanding I/O to disk
2. Execute the final delta sync:
...
4. Shutdown source VMs
5. Network cutover. Redirect user traffic to the new environment in VPC. This step depends on how you have been connecting to your IBM Cloud VMware VCF-Autonmated instance but could include:
