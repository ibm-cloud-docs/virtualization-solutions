...
The majority of VMs hosted on IBM Cloud VCF-Automated are connected to NSX overlay segments which **do not** have native access to the IBM Cloud Classic networks. Additionally as we want the the target VSIs to have the **same IP addresses** as the source VMs we have to ensure that the NSX overlay segment and the target VSI VPC subnets are isolated. We accomplish this with the following RMM features:
...
2. **Reverse SSH Tunnel**: Allows the target VSI to reach the source VM through the RMM server
3. **Key-based Authentication**: Target VSI uses RMM's SSH key to authenticate to the source VM
4. **Data sync via the Tunnel**: The target VSI pulls data from the source VM through the secure SSH tunnel
5. **Bootloader Installation**: RMM makes target bootable with proper GRUB configuration
...
2. When the source VM replies, it appears to come from the NAT IP `10.194.177.82`

When RMM tries to reach `10.194.177.82`:

* Step 1: RMM sends packet
    * SRC: 10.68.70.11
    * DST: 10.194.177.82

* Step 2: VPC Routes to Transit Gateway which routes to Classic network which knows that:
    * "10.194.177.82 is in subnet 10.194.177.82/30"
    * "Route this subnet to 10.134.54.62"

* Step 3: Packet delivered to bridge server
    * Arrives on ens224 (10.134.54.62)
    * DST: 10.194.177.82 (unchanged)

* Step 4: Bridge's iptables DNAT
    * `iptables -t nat -A PREROUTING -i ens224 -d 10.194.177.82 -j DNAT --to-destination 192.168.10.11`

* Step 5: Packet forwarded to source VM
    * SRC: 10.68.70.11
...
3. **Target VSI → Source VM** (through tunnel)
...
### 2. **Capture** (Store-and-Forward Approach)
...
### 4. **Direct Assign** (Also called Flex Sync or Host Sync)
...
**Process:**
- Combines Capture + Assign into single operation
- Performs all Discover functions
- Provisions target server (or uses existing)
- Prepares target server
- Replicates **directly** from source VM to target VSI
- Network connection can still route through RMM (no direct Source-to-Target connection required)

### 5. **Sync** (Delta Synchronization)
{: #virt-sol-vpc-migration-design-rmm-guide-ops-sync}

**Purpose:** Update target with only changed data from the source VM
**Process:**
- Takes snapshot on source VM (LVM or VSS)
- Calculates **delta** (only changed files)
- Transfers only changed data to Target
- Updates either:
...
2. **Reformats the disk** while in microkernel
3. **Recreates partition structure** matching the source VM
4. **Creates Logical Volumes** to create the LVM structure
5. **Transfers the source VM data** to the target VSI
6. **Injects necessary drivers** for the target hardware
7. **Configures GRUB** to boot the actual OS
8. **Reboots** into the replicated OS
...
**Step 1: RMM Discovers the Source VM**
...
**Step 2: RMM Discovers the Target VSI**
...
**Step 3: RMM Creates Reverse SSH Tunnel to Target VSI**
...
2. Creates a listening port (23) ON the target
3. Any connection to `localhost:23` on target gets forwarded:
...
2. Port 23 stops listening on target

**Step 9: Remove Utilities**
