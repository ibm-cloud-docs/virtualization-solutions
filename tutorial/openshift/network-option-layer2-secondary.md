...
This tutorial shows how to create an example three-tier application by using Cluster User-Defined Network (CUDN) Layer 2 secondary networks and namespaces on {{site.data.keyword.redhat_openshift_full}} Kubernetes Service on IBM Cloud. The example demonstrates how to attach virtual servers running on {{site.data.keyword.redhat_openshift_notm}} Virtualization to Layer 2 secondary networks that provide isolated network segments without external access. For more information on network types, see [OVN networking in OpenShift for vSphere administrators](/docs/virtualization-solutions?topic=virtualization-solutions-virt-sol-network-options-overview). 
{: shortdesc}

## Overview
{: #layer2-secondary-overview}

This guide demonstrates how to create an example three-tier application using CUDN Layer 2 secondary networks and namespaces. The overall setup creates isolated network segments with no external access. Layer 2 secondary networks provide network isolation between different application tiers while allowing controlled communication within and across namespaces.
...
![Layer 2 secondary isolated network setup](../../images/openshift/openshift-virtualization-layer2-secondary-basic.png "Diagram of the Layer 2 secondary isolated network setup across the white and black namespaces"){: caption="Layer 2 secondary isolated network setup" caption-side="bottom"}

Each code block in this guide can be copied to a file and applied with `oc apply -f <file-name>.yml`. Some code blocks might cross page boundaries in PDF and Word formats, and some long lines might wrap in those formats.
{: tip}

## Step 0: Create namespace and CUDN Layer 2 secondary network
{: #layer2-secondary-step-0-white-namespace}

Layer 2 secondary networks provide isolated network segments that can be attached to virtual servers as additional network interfaces. Unlike Layer 2 primary networks, secondary networks do not require special namespace labels and can coexist with the default pod network.
...
## Step 1: Create another namespace and CUDN Layer 2 secondary network
{: #layer2-secondary-step-1-black-namespace}
...
## Step 2: Add virtual servers to the namespaces
{: #layer2-secondary-step-2-add-vms}

The following manifests create the three virtual servers that are listed in the overview table:
...
Each virtual server attaches only Layer 2 secondary networks. These virtual servers have no external access and must be accessed through the OpenShift web console.
...
The `jail-db00` virtual server attaches to both Layer 2 secondary networks. It is reachable from `jail-web00` on the `white-isolated` network and from `jail-app00` on the `black-white-isolated` network.
...
## Step 3: Test the network setup
{: #layer2-secondary-step-3-test}

From the OpenShift web console, log in to the virtual servers to test network connectivity.
...
1. **Test connectivity within the white-isolated network**
   ...
   Ping from `jail-web00` to `jail-db00`. This test should succeed because both virtual servers are on the same `white-isolated` network.

2. **Test connectivity within the black-white-isolated network**

   Ping from `jail-db00` to `jail-app00`. This test should succeed because both virtual servers are on the same `black-white-isolated` network that spans the `white` and `black` namespaces.

3. **Test network isolation between different Layer 2 networks**

   Ping from `jail-web00` to `jail-app00`. This test should fail because these virtual servers are on different Layer 2 secondary networks (`white-isolated` and `black-white-isolated`) with no routing between them.

These tests demonstrate the network isolation provided by Layer 2 secondary networks. Virtual servers can communicate within the same Layer 2 network segment, but traffic cannot flow between different Layer 2 networks without additional routing configuration.
{: note}
...
- Create additional Layer 2 secondary networks for different application tiers
