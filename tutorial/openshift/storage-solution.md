...
### Worker node count and topology
{: #number-of-worker-nodes}

The failure domain topology used by the {{site.data.keyword.cloud_notm}} ODF add-on depends on your cluster configuration:

- **Multi-zone clusters (3 availability zones):** The failure domain is set to `zone`. Worker nodes are distributed across zones, and ODF must grow in multiples of 3 to keep zones balanced. For optimal availability, performance, and data safety, use 3, 6, or 9 nodes in the ODF storage cluster — each zone receives the same number of nodes.
- **Single-zone clusters or clusters with fewer than 3 availability zones:** Flexible scaling is automatically enabled and the failure domain is set to `host`. You can start with 3 nodes and add nodes one at a time.
...
For multi-zone clusters: adding nodes in counts that are not multiples of 3 creates a zone imbalance. With 4 nodes, one zone gets 2 nodes while the others get 1 each, resulting in uneven OSD weight distribution, suboptimal data placement, uneven usage, and partially idle OSDs.

For multi-zone clusters, always scale in multiples of 3 to maintain a balanced zone topology.
{: important}
...
You can verify node topology assignments on your cluster by running the following command:
...
- Option A – Use the entire worker pool
   ...
   - For multi-zone clusters, verify that the pool contains 3, 6, or 9 bare metal nodes (multiples of 3). For single-zone clusters, a minimum of 3 nodes is required.

- Option B – Select specific nodes
   - If the pool has more nodes, or you want to reserve some nodes for compute-only workloads, select the nodes to participate in ODF. For multi-zone clusters, select nodes in multiples of 3; for single-zone clusters, any count of 3 or more is valid.
...
For bare-metal deployments with local NVMe drives, use the **Performance** resource profile. Bare-metal nodes with 8 or more NVMe drives per node create a high OSD count per host. With the Balanced profile, Ceph daemon CPU and memory limits are often reached before the underlying NVMe hardware is saturated, which caps IOPS and increases latency. The Performance profile is the minimum recommended profile for production {{site.data.keyword.redhat_openshift_notm}} Virtualization workloads on bare-metal.
{: important}
...
When you select nodes during StorageSystem creation, avoid selecting **all nodes** in the cluster. Selecting all nodes creates a LocalVolumeSet (LVS) with no `nodeSelector`. Any worker node that is added to the cluster in the future will be automatically detected by ODF, even if it is not intended for storage. Nodes that are detected unexpectedly must be manually removed from the LVS. To prevent this, select only the nodes in your dedicated storage worker pool, or ensure that those nodes carry the `cluster.ocs.openshift.io/openshift-storage` label before StorageSystem creation.
{: important}
...
- ODF storage cluster uses a minimum of 3 nodes; multi-zone clusters must use multiples of 3 (3, 6, 9, …) to maintain zone balance
...
### Ceph performance tuning for NVMe bare-metal
{: #ceph-performance-tuning}

The default Ceph configuration is tuned for general workloads. The following parameter values are validated on the `mx2d.metal.96x768` bare-metal profile and significantly increase IOPS and reduce latency for VM disk workloads on that profile. If you are using a different bare-metal profile, treat these as a starting reference and adjust the values based on the NVMe drive count and available CPU of your specific profile.

| Parameter | Recommended value | Rationale |
| --------- | ----------------- | --------- |
| `osd_memory_target` | `8589934592` (8 GB) to `12884901888` (12 GB) | Increases the BlueStore cache available to each OSD. More cache reduces read amplification and improves random read IOPS. Default is 4 GB, which is insufficient for high-density NVMe nodes. |
| `osd_op_num_shards_ssd` | `16` | Each shard handles a queue of I/O operations. Increasing from 8 to 16 shards on high-core-count bare-metal nodes allows more parallel request processing and reduces queue depth per shard. |
| `osd_op_num_threads_per_shard_ssd` | `2` | Controls the number of worker threads per shard. Increasing this value alongside shard count improves concurrent I/O throughput on NVMe drives. |
| `bluestore_prefer_deferred_size_ssd` | `0` | Disables deferred writes for NVMe. Deferred writes cause a double-write through the WAL (write-ahead log), which adds overhead. NVMe drives have fast enough random write performance to make deferred writes counterproductive. |
| RocksDB `rocksdb_write_buffer_size` | `268435456` (256 MB) | Increases the RocksDB memtable size. A larger write buffer absorbs bursts of metadata writes (common during VM provisioning and snapshot operations) before flushing to disk, reducing write stalls. |
| RocksDB `rocksdb_max_write_buffer_number` | `16` to `32` | Controls the maximum number of write buffers in memory. Reducing from the default of 64 to 16–32 on NVMe is sufficient and reduces memory pressure. |
| RocksDB `rocksdb_max_background_jobs` | `12` to `16` | Controls the number of concurrent compaction and flush threads. Increasing this value on NVMe nodes prevents RocksDB compaction from becoming a bottleneck under sustained write load. |
{: caption="Recommended Ceph tuning parameters for NVMe bare-metal"}

Apply each parameter by using the `ceph config set` command from the Ceph toolbox pod:
...
After applying, verify that the configuration was accepted:
...
OSD pods do not need to be restarted for `ceph config set` changes. Ceph applies the configuration dynamically. However, BlueStore cache changes (`osd_memory_target`) take full effect only after each OSD pod is recycled. You can recycle OSD pods one at a time during a maintenance window without interrupting I/O.
{: note}

**Benchmark reference:** Internal testing on a 3-node `mx2d.metal.96x768` cluster with 200 VMs and unlimited IOPS showed that the combination of 16 shards, 8 GB OSD memory, and a 256 MB RocksDB write buffer achieved approximately **194,000 IOPS** and **758 MB/s** throughput. Increasing shards from 8 to 16 consistently produced the largest single improvement in IOPS across all test configurations.
...
1. [Add worker nodes to your VPC cluster](/docs/openshift?topic=openshift-add-workers-vpc). For multi-zone clusters where the storage cluster spans 3 availability zones, add worker nodes in multiples of 3 to maintain zone balance (for example, 3, 6, or 9). For single-zone clusters with flexible scaling enabled, you can add nodes one at a time.

2. After the nodes are added, register them with ODF. If ODF runs on all worker nodes in your cluster, new nodes are added to the storage topology automatically. If ODF runs on only a subset of worker nodes, proceed to the next step.
...
6. Verify that the new worker nodes are added and evenly distributed across each zone (for multi-zone clusters) or appear as individual host buckets (for single-zone flexible-scaling clusters), along with the corresponding number of OSDs assigned to each node.
...
The {{site.data.keyword.cloud_notm}} ODF add-on uses different failure domain topologies depending on your cluster configuration:

- **Multi-zone clusters (3 availability zones):** The failure domain is set to `zone`. OSDs are provisioned in multiples of 3, one set per zone, to maintain data replication and high availability across zones. The storage cluster must grow in multiples of 3 to keep zones balanced.
- **Single-zone clusters or clusters with fewer than 3 availability zones:** Flexible scaling is automatically enabled. The failure domain is set to `host`, meaning each individual node is a distinct failure domain. You can add one node at a time and scale storage granularly.

Starting with ODF 4.21, flexible scaling behavior is determined automatically at initial deployment based on the cluster topology and cannot be changed afterward.

In a single-zone or flexible-scaling deployment, a replica-3 pool survives the loss of any single host. In a multi-zone deployment, a replica-3 pool survives the loss of an entire zone. Evaluate whether your cluster topology and its associated fault tolerance meet your resilience requirements before deploying ODF in production.
{: important}

For the full set of add-on parameters and console-based installation steps, see [Deploying OpenShift Data Foundation on VPC clusters](/docs/openshift?topic=openshift-deploy-odf-vpc).

**Performance during node expansion:** Adding a node to a flexible-scaling ODF cluster triggers Ceph data rebalancing. In the internal test described below, IOPS and throughput remained stable while write latency temporarily increased. In internal testing on a 3-node cluster with 100 VMs at 50,000 IOPS, the following results were observed:

| Stage | IOPS | Throughput | Read latency | Write latency |
| ----- | ---- | ---------- | ------------ | ------------- |
| Before adding node | 50,000 | 195 MB/s | 0.69 ms | 1.37 ms |
| During adding node | 50,000 | 195 MB/s | 1.24 ms | 2.22 ms |
| After adding node | 50,000 | 195 MB/s | 0.67 ms | 1.22 ms |
{: caption="Performance impact of adding a node with flexible scaling enabled"}

Write latency returns to baseline after rebalancing completes. Plan node additions during periods of lower VM activity if your workloads are sensitive to write latency spikes.
